...
public class SwipeListViewTouchListener implements View.OnTouchListener {
	 ...
	 */
	/*
	 * private void generateRevealAnimate(final View view, final boolean swap, final boolean swapRight, final int
	 * position) { int moveTo = 0; if (opened.get(position)) { if (!swap) { moveTo = openedRight.get(position) ? (int)
	 * (viewWidth - rightOffset) : (int) (-viewWidth + leftOffset); } } else { if (swap) { moveTo = swapRight ? (int)
	 * (viewWidth - rightOffset) : (int) (-viewWidth + leftOffset); } }
	 * 
	 * animate(view) .translationX(moveTo) .setDuration(animationTime) .setListener(new AnimatorListenerAdapter() {
	 * 
	 * @Override public void onAnimationEnd(Animator animation) { swipeListView.resetScrolling(); if (swap) { boolean
	 * aux = !opened.get(position); opened.set(position, aux); if (aux) { swipeListView.onOpened(position, swapRight);
	 * openedRight.set(position, swapRight); } else { swipeListView.onClosed(position, openedRight.get(position)); } }
	 * resetCell(); } }); }
	 */
	...
	public boolean onTouch(View view, MotionEvent motionEvent) {
		...
		case MotionEvent.ACTION_DOWN: {
			...
			for (int i = 0; i < childCount; i++) {
				...
				// dont allow swiping if this is on the header or footer or IGNORE_ITEM_VIEW_TYPE or enabled is false on
				// the adapter
